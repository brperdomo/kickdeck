...
async function updateRefundTemplate() {
  try {
    console.log('Checking if refund template exists...');
    ...
    if (!existingTemplate || existingTemplate.length === 0) {
      console.log('Refund template not found. Please run the create-email-templates.js script first.');
      return;
    }
    
    console.log('Updating refund template to handle partial refunds...');
...
<body>
  <div class="container">
    ...
    console.log('Refund email template successfully updated to support partial refunds!');
  } catch (error) {
    console.error('Error updating refund template:', error);
  }
}

// Run the update
updateRefundTemplate();