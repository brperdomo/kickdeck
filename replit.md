...
- **FIELD EXCLUSION SYSTEM REBUILT**: Completely rebuilt field removal functionality with tournament-level field exclusion system, FieldExclusionManager component, and secure API endpoints for safe field management
...
- **Core Features**:
    ...
    - **Tournament-Specific Field Management System**: Comprehensive field configuration system allowing tournament directors to set field sizes (3v3 through 11v11) per-tournament rather than per-complex. Features drag-and-drop field ordering interface with tournament-specific field size dropdowns in the Master Scheduler's Field Order tab. Includes robust field exclusion system with constraint validation, bulk configuration updates, and persistent field ordering integration.
    - **Field Exclusion and Removal System**: Tournament-level field exclusion system allowing complete removal of fields from tournament usage without affecting other tournaments. Features FieldExclusionManager component, secure API endpoints (/api/admin/events/:eventId/fields/:fieldId/exclude), restoration capabilities, and comprehensive safety controls with warning dialogs.
