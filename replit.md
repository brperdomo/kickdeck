...
- **Core Features**:
    ...
    - **Bracket Assignment Interface**: Manual team-to-bracket assignment system allowing tournament directors to control which specific teams are placed in Bracket A vs Bracket B within flights, with auto-balance functionality for even distribution.
    - **Field Management System**: Comprehensive field sorting functionality with drag-and-drop interface allowing tournament directors to customize field display order in the Master Scheduler's Calendar Grid. Includes field deletion with constraint validation, bulk sort order updates, and persistent field ordering integration.
