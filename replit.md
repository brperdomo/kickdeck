...
- **FLIGHT VS BRACKET CONSOLIDATION COMPLETE (August 10, 2025)**: Fixed critical data structure issue where Group A/B entries were incorrectly stored as separate flights. Consolidated all Group A/B brackets back into their parent flights, moving all teams from Group structures (e.g., "Boys 2016 - Nike Premier Group A/B") into the main flight brackets (e.g., "Boys 2016 - Nike Premier"). Deleted 26 redundant Group bracket entries and updated bracket-assignments API to properly recognize flight vs bracket hierarchy. Tournament structure now matches flight configuration CSV with clean flight-level organization.
