...
- **Group of 8 Format Fix (Aug 2025)**: COMPLETED - Corrected dual bracket system implementation with automatic team distribution. Group of 8 now properly generates two separate 4-team round-robin brackets (Bracket A vs Bracket B) with no cross-bracket play except championship final. Fixed TypeScript compilation errors, implemented automatic 4v4 team distribution for unassigned teams, corrected game generation logic to prevent crossplay matchups, and validated proper bracket assignment across all flights. System now automatically distributes 8 teams into two balanced 4-team brackets when no manual assignments exist.
