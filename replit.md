...
- **CRITICAL FIX - Flight Duplication Bug Resolved**: Fixed massive data integrity issue where Game Format Engine showed 65+ "duplicate" flights needing configuration. Root cause was incorrect team-to-bracket assignments - teams from multiple age groups (U9, U13, U17) were incorrectly assigned to single brackets, causing API to return confusing duplicate entries like "U13 Boys Nike Elite A/B/C". Corrected all team assignments to match proper age group and gender brackets, restoring tournament structure integrity.
- **Database Team Assignment Correction**: Systematically moved incorrectly placed teams to their proper brackets based on age group and gender matching. Each team now resides in exactly one appropriate bracket for their competitive level.
- **Flight-Formats API Data Integrity**: Resolved API query confusion that was returning incorrect age group metadata for brackets due to cross-contaminated team assignments. Now returns accurate flight information for Game Format Engine.
