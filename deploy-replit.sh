...
async function startServer() {
  ...
  console.log(`Frontend build found: ${hasFrontend ? '✅' : '❌'}`);
  ...
  try {
    ...
    console.log('⚠️ Setting up fallback server due to error');
    
    // Setup static file serving regardless of the error
    if (hasFrontend) {
      ...
      app.get('*', (req, res, next) => {
        if (req.path.startsWith('/api/')) {
          return next(); // Let the API routes handle this
        }
    ...
    // Basic API endpoints that should always work
    ...
    app.use('/api/*', (req, res) => {
      res.status(404).json({ 
        ...
        status: 'error',
        message: 'The server is running in fallback mode due to startup errors'
      });
    });
    
    // Final fallback error page if frontend isn't available
    if (!hasFrontend) {
      app.use('*', (req, res) => {
