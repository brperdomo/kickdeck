/**
 * Test SendGrid Member Welcome Email
 * 
 * This script tests sending a member welcome email using SendGrid's dynamic templates
 * and verifies the merge fields are working correctly.
 * 
 * Usage:
 *   node test-sendgrid-member-welcome.js test@example.com
 */
...
async function testMemberWelcomeEmail() {
  ...
  if (!recipientEmail) {
    ...
    console.log('Usage: node test-sendgrid-member-welcome.js test@example.com');
    process.exit(1);
  }

  // Use the template ID from our database
  const templateId = process.argv[3] || 'd-6064756d74914ec79b3a3586f6713424';
  
  console.log(`Using template ID: ${templateId}`)

  try {
    console.log(`Testing SendGrid member welcome email to ${recipientEmail} using template ${templateId}`);
...
testMemberWelcomeEmail().catch(err => {
