  ...
  React.useEffect(() => {
    if (existingParameters) {
      ...
      if (existingParameters.autoGenerated) {
        toast({
          title: "🤖 Fully Automated Tournament Configuration",
          description: `Auto-configured ${existingParameters.ageGroups?.length || 0} age groups, ${existingParameters.gameFormats?.length || 0} game formats, and ${existingParameters.fields?.length || 0} fields. Tournament ready for one-click scheduling!`
        });
      }
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {existingParameters?.autoGenerated && (
            <Alert className="mb-4 bg-green-50 border-green-200">
              <CheckCircle className="h-4 w-4 text-green-600" />
              <AlertDescription className="text-green-800">
                <strong>🤖 FULLY AUTOMATED CONFIGURATION COMPLETE!</strong><br/>
                Auto-configured {existingParameters.ageGroups?.length || 0} age groups, {existingParameters.gameFormats?.length || 0} game formats, 
                and {existingParameters.fields?.length || 0} fields based on your tournament data. 
                {existingParameters.totalTeams && ` Ready to schedule ${existingParameters.totalTeams} teams!`}
                <div className="mt-3">
                  <Button 
                  ...
                  >
                    {saveParametersMutation.isPending ? (
                      <>
                        ...
                        Saving Automated Configuration...
                      </>
                    ) : (
                      <>
                        <Save className="h-4 w-4 mr-2" />
                        Accept Automated Configuration & Proceed
                      </>
                    )}
                  </Button>
                </div>
