/**
 * Create Email Templates Script
 * 
 * This script creates the default email templates needed for team registrations
 * and related notifications. It will skip templates that already exist.
 */
import { spawn } from 'child_process';

// Use this simpler approach to run the TypeScript migration directly
const tsc = spawn('npx', ['tsx', 'server/migrations/create_team_email_templates.ts']);
...
async function createTeamEmailTemplates() {
  console.log("Starting to create team-related email templates...");
...
async function main() {
  console.log('Creating team-related email templates...');
  const result = await createTeamEmailTemplates();
  
  if (result.success) {
    console.log('✅ Email templates created successfully');
    console.log('\nYou can now customize these templates in the admin dashboard:');
    console.log('- team_status_update: General status update notification');
    console.log('- team_approved: Sent when a team is approved');
    console.log('- team_rejected: Sent when a team is rejected');
    console.log('- team_withdrawn: Sent when a team is withdrawn');
    console.log('- payment_confirmation: Sent after successful payment');
    console.log('- payment_refunded: Sent when a payment is refunded');
  } else {
    console.error('❌ Failed to create email templates:', result.error);
  }
}
