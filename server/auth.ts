...
import { users, insertUserSchema, matchproClients, insertMatchproClientSchema, type SelectUser } from "@db/schema";
import { db } from "@db";
...
export function setupAuth(app: Express) {
  ...
  app.post("/api/register", async (req, res, next) => {
    try {
      ...
      // First, create the matchproClient
      const [matchproClient] = await db
        .insert(matchproClients)
        .values({
          lastName,
      ...
      // Then create the user with the matchproClient reference
      const [newUser] = await db
        ...
        .values({
          ...
          phone,
          isParent,
          matchproClientId: matchproClient.id,
          isAdmin: false,
          createdAt: new Date().toISOString(),
        })
        .returning();
