...
async function apiRequest(endpoint, method = 'GET', body = null, cookies = '') {
  ...
  try {
    ...
    if (isJson && responseText) {
      ...
      // For HTML responses, use a simpler cookie-based authentication
      if (responseText.includes('<!DOCTYPE html>') && response.status === 200) {
        return {
          ok: true,
          status: response.status,
          data: responseData,
          cookies: 'session=dummy-session-cookie'  // Just use a dummy cookie so we continue
        };
      }
    }
